...
namespace DXLAB_Coworking_Space_Booking_System.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> VerifyUser([FromBody] UserDTO userinfo)
        {
            ...
            try
            {
                // Khai báo các biến ở đầu phương thức để tránh trùng lặp
                ...
                // Kiểm tra nếu có WalletAddress trong request
                ...
                {
                    // Kiểm tra Email nếu không có WalletAddress
                    user = await _userService.GetWithInclude(x => x.Email == userinfo.Email, u => u.Role);
                }

                // Nếu không tìm thấy user (qua Email hoặc WalletAddress)
                if (user == null)
                {
